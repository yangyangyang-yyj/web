...
import LessonInterview from '../components/LessonInterview.vue'
...
const activeIndex = computed(() =>
  lessons.findIndex((l) => l.id === props.id) + 1,
)

const isInterview = computed(() => activeLesson.value?.category === 'interview')
...
<template>
  <main v-if="activeLesson" ref="mainRef" class="main">
    ...
    <div v-if="activeLesson" class="content">
      <LessonInterview
        v-if="isInterview"
      ...
      <template v-else>
        <LessonKnowledge
          v-if="activeLesson.knowledge"
          :knowledge="activeLesson.knowledge"
        />
        <LessonExample
          ...
          :files="activeLesson.example.files || []"
          :preview-html="activeLesson.example.previewHtml || ''"
...
.category-badge.echarts {
  background: #fee2e2;
  color: #b91c1c;
}

.category-badge.datav {
  background: #cffafe;
  color: #0e7490;
}
